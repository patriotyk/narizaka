...
def run():
    ...
    parser.add_argument('-ws',  help='Weakly supervised mode, use ASR transcription for annotation(doesn\'t support speaker id data structure yet)',  action='store_true',  default=False)
    ...
    if not input_data.is_empty():
        ...
        if len(input_data.transcribed_books):
            ...
            for book_pair in input_data.transcribed_books:
                if book_pair.audio_book.speaker_id:
                    print(book_pair.audio_book.speaker_id, book_pair.audio_book.filename)
                else:
                    print(book_pair.audio_book.filename)
